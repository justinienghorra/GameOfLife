...
def alreadyVisited(newState, visitedList, length):
    found = False
    i=0
    while not found and i<length: #on s'arrete si on a fait length decalages
        ...
        #print("B : ", newState)
        if newState in visitedList: #arret si cette nouvelle grille a deja ete ajoutee precedemment
            found = True
        i+=1
    i=0
...
def alreadyVisitedWithOldState(newState, visitedList, length):
    found = False
    i=0
    while not found and i<length: #on s'arrete si on a fait length decalages
        item = newState.pop()
	    ...
	    for e2 in lst:
	        for e3 in lst:
	            for e4 in lst:
	                ...
	                if not alreadyVisited(newState, finalElements, length):
